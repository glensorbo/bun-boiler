 ...
 * Bun replaces Bun.env.BUN_PUBLIC_* with literal values at bundle time
 * in both dev (via [serve.static] env in bunfig.toml) and production builds.
 *
 * Never access Bun.env.BUN_PUBLIC_* directly outside this file.
 */
export const config = {
  openpanel: {
    clientId: Bun.env.BUN_PUBLIC_OPENPANEL_CLIENT_ID ?? null,
    apiUrl: Bun.env.BUN_PUBLIC_OPENPANEL_API_URL ?? null,
  },
  otel: {
    /**
     ...
     */
    serviceName: Bun.env.BUN_PUBLIC_OTEL_SERVICE_NAME ?? null,
  },
  /** Whether self-service signup is enabled. Set BUN_PUBLIC_ENABLE_SIGNUP=true to enable. */
  enableSignup: Bun.env.BUN_PUBLIC_ENABLE_SIGNUP === 'true',
} as const;
