...
| `signAuthToken.ts`        | `signAuthToken`        | Signs a short-lived (15m) auth JWT (`tokenType: 'auth'`)       |
| `verifyToken.ts`          | `verifyToken`          | Verifies and decodes a JWT, returns `ErrorOr<AppJwtPayload>`   |
