...
| `signSignupToken.ts`      | `signSignupToken`      | Signs a short-lived (1h) signup JWT (`tokenType: 'signup'`) with `name` in payload         |
| `signAuthToken.ts`        | `signAuthToken`        | Signs a short-lived (15m) auth JWT with `role` and `name` claims                           |
| `verifyToken.ts`          | `verifyToken`          | Verifies and decodes a JWT; extracts `sub`, `email`, `name`, `role` (defaults to `'user'`) |
...
```ts
signAuthToken(userId: string, email: string, role: UserRole, name: string): Promise<string>
signSignupToken(userId: string, email: string, name: string): Promise<string>
verifyToken(token: string): Promise<ErrorOr<AppJwtPayload>>
// AppJwtPayload: { sub, email, name, tokenType, role }
```
